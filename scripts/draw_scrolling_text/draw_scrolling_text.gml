...
function draw_scrolling_text() {
	...
	if !variable_instance_exists(id,"textChar"){
		...
		prevChar = 0; //The previous character of the text to add to the string.
	}
	...
	if (currentText!=undefined){
		...
		textWait = undefined;
		textCut = undefined;
		ds_list_clear(oGame.text_queue);
		instance_destroy();
	}
	...
	if (currentText!=undefined){
		...
		if ((floor(textChar) - prevChar) >= 1) {
			audio_play_sound(TextScrollSound,0,false);
		}
		prevChar = floor(textChar);
	}
